...
def build_embedding_matrix(word2idx, embed_dim, type):
    ...
    if os.path.exists(embedding_matrix_file_name):
        ...
        fname = 'glove/chinese_wiki_embeding20000.txt'
...
class ABSADatesetReader:
    ...
    def __read_data__(fname, tokenizer):
        ...
        fin.close()
        fin = open(fname + '.ef_graph', 'rb')
        idx2ef_graph = pickle.load(fin)
        fin.close()
        ...
        for i in range(0, len(lines), 3):
            ...
            polarity = int(polarity) + 1  # 标签极性
            distance_graph = idx2gragh[i]
            ef_graph = idx2ef_graph[i]
    ...
    def __init__(self, dataset='data', embed_dim=300):
        print("preparing {0} dataset ...".format(dataset))
        fname = {
            'mooc': {
                'train': './datasets/mooc/train.raw',
                'test': './datasets/mooc/test.raw'
            },
            'douban': {
                'train': './datasets/douban/train.raw',
                'test': './datasets/douban/test.raw'
            },
            'data': {
                'train': './datasets/data/train.raw',
                'test': './datasets/data/test.raw'
            },
            'twitter': {
